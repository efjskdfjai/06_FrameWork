...
/* console.log("ajax-mainjs loaded.");
...
function getTotalCount() { // 함수의 정의
  ...
  fetch("/ajax/totalCount") // 서버로 "/ajax/totalCount"로 GET 요청
    ...
    // 두 번째 then (첫 번째에서 return (변환)된 데이터를 활용하는 역할)
    .then(result => { // 첫 번째 콜백함수가 완료된 후 호출되는 콜백함수
      // 변환된 텍스트 데이터(result)를 받아서
      // 콘솔에 단순 출력
      // 매개변수 result : 첫 번째 콜백함수에서 반환된 Promise 객체의 PromiseResult값
      // == result 매개변수로 받아서 처리
...
const selectTodoList = () => {
  fetch("/ajax/selectList")
    ...
    .then(todoList => {
      // 매개변수 todoList;
      // 첫 번째 then에서 resp.text() / resp.json()
      // 단순 텍스트이거나. JS Object 일 수 있음

      // 만약 resp.text() 사용했다면 문자열(JSON 그대로 노출)
      ...
      // - JS Object 타입을 string 형태의 JSON 데이터로 변환0
      ...
      for (let todo of todoList) { // 향상된 for문
        ...
        for(let key of arr) {
          ...
          tr.append(td); // tr의 마지막 요소 현재 id 추가하기
